...
//20191009
bool RecordDaoImp::insertRecordInfo(const QString &number, const QString &type, const QString &placename, const QString &receiver, const QString &carnumber, const QString &dirver, const QString &ticketTime, const QString &originalTime, const QString &watcher, const QString &modefyTime, const QString &modefier, double totalweight, double carweight, double thingsweight, double price,const QString& otherInformation, const QString &record_flag)
{
    // qDebug()<<"#[数据库操作]-记录表-增加";
     ...
     //------------------------------------------------------------------
     QSqlQuery query;
     query.prepare("insert into tb_record(\
        number, type, placename, receiver, carnumber, dirver, totalweight, carweight, thingsweight, price, ticketTime, originalTime, watcher, modefyTime, modefier,otherInformation, record_flag\
                   )values(\
       :number,:type,:placename,:receiver,:carnumber,:dirver,:totalweight,:carweight,:thingsweight,:price,:ticketTime,:originalTime,:watcher,:modefyTime,:modefier,:otherInformation,:record_flag\
                 );");
     ...
     query.bindValue(":otherInformation",(const QVariant)otherInformation);//20191009
     ...
     if(!ret)
     {
         qDebug()<<"sql---新增---失败"<<query.lastError();
         return false;
     }
     qDebug()<<"sql---新增---成功";
     return true;
...
bool RecordDaoImp::selectallRecordInfo(QSqlQueryModel *model)
{
    ...
    model->setQuery("select * from tb_record order by ticketTime asc;");
    model->setHeaderData(0, Qt::Orientation::Horizontal, "流水号");
    model->setHeaderData(1, Qt::Orientation::Horizontal, "品类");
    model->setHeaderData(2, Qt::Orientation::Horizontal, "场地");
    model->setHeaderData(3, Qt::Orientation::Horizontal, "收货人");
    model->setHeaderData(4, Qt::Orientation::Horizontal, "车牌号");
    model->setHeaderData(5, Qt::Orientation::Horizontal, "驾驶员");
    model->setHeaderData(6, Qt::Orientation::Horizontal, "毛重(吨)");
    model->setHeaderData(7, Qt::Orientation::Horizontal, "皮重(吨)");
    model->setHeaderData(8, Qt::Orientation::Horizontal, "净重(吨)");
    model->setHeaderData(9, Qt::Orientation::Horizontal, "单价(元)");
    model->setHeaderData(10, Qt::Orientation::Horizontal, "票据时间");
    model->setHeaderData(11, Qt::Orientation::Horizontal, "原始时间");
    model->setHeaderData(12, Qt::Orientation::Horizontal, "过磅人");
    model->setHeaderData(13, Qt::Orientation::Horizontal, "修改时间");
    model->setHeaderData(14, Qt::Orientation::Horizontal, "修改人");
    model->setHeaderData(15, Qt::Orientation::Horizontal, "票据类型");
    model->setHeaderData(16, Qt::Orientation::Horizontal, "备注信息");//20191009
...
//条件查找
//20191009
bool RecordDaoImp::selectRecordInfo_date(const QString &date_S, const QString &date_E, const QString &type, const QString &place, const QString &car, QSqlQueryModel *model)
{
    // qDebug()<<"#[数据库操作]-记录表-条件查询";//substr(time,0,8)//字符串截取查询（还有很多用法）
     ...
     model->setHeaderData(13, Qt::Orientation::Horizontal, "修改时间");
     model->setHeaderData(14, Qt::Orientation::Horizontal, "修改人");
     model->setHeaderData(15, Qt::Orientation::Horizontal, "备注信息");
     model->setHeaderData(16, Qt::Orientation::Horizontal, "票据类型");
     dbhelper->destoryconnect();

     qDebug()<<"selectRecordInfo_date";
     return true;
}
...
bool RecordDaoImp::updateRecordInfo(const QString &number,
                                    ...
                                    const QString &record_flag)
{
    ...
    dbhelper->createconnect();
    QSqlQuery query;
    query.prepare("update tb_record set \
                  placename = ?,\
                  receiver = ?,\
                  carnumber = ?,\
                  dirver = ?,\
    ...
    query.bindValue(11,thingstype,QSql::Out);
    query.bindValue(12,otherInformation,QSql::Out);
    query.bindValue(13,number,QSql::Out);
...
bool RecordDaoImp::deleteRecordInfo(const QString &number)
{
    ...
    dbhelper->createconnect();
    QSqlQuery query;
    query.exec("delete from tb_record where number = ?;"); //账号
...
bool RecordDaoImp::isOneExist(const QString &number)
{
    ...
    //------------------------------------------------------------------
    QSqlQuery query;
    query.prepare("select * from tb_record where number = :number;");
    query.bindValue(":number",(const QVariant)number);
    bool ret = query.exec();
